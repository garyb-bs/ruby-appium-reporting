...
build_url = "https://#{ENV["BROWSERSTACK_USERNAME"]}:#{ENV["BROWSERSTACK_ACCESS_KEY"]}@api.browserstack.com/app-automate/builds.json?limit=#{buildLimit}&offset=#{offset}"
build_results = RestClient.get(build_url)
build_results_json = JSON.parse(build_results.body)
build_details = build_results_json[0]['automation_build']

build_id = build_details['hashed_id']
puts build_id
